...
st.set_page_config(
    page_title=f"Chatbot (Tokens & Cost) - {MODEL_NAME}", page_icon="🪙"
)
...
async def get_anthropic_response_stream_with_usage(
    ...
    if message_history_for_api:
        ...
        ]  # Deep copy to avoid modifying original session state
    ...
    try:
        async with client.messages.stream(
            ...
            else message_history_for_api,  # Use modified list if available
            model=MODEL_NAME
        ...
        st.session_state._current_stream_full_text = (
            full_response_text  # Store potentially partial text
        )
...
if (
    "session_total_hypothetical_cost" not in st.session_state
):  # New state for hypothetical cost
        ...
        if (
            ...
            for token_type in token_types_ordered:
                if token_type in usage and usage[token_type] > 0:
                    details.append(
                        f"{token_type.replace('_', ' ').title()}: {usage[token_type]}"
                    )
...
if prompt := st.chat_input("What would you like to ask?"):
    ...
    with st.chat_message("assistant"):
        ...
        if (
            displayed_response_text
        ):  # This will be non-empty if anything was yielded
            current_cost = 0.0
            ...
            if is_successful_api_response:
                ...
                ) * model_specific_costs["input_tokens"]
                current_turn_hypothetical_cost += (
                    getattr(usage_info, "output_tokens", 0) / 1_000_000
                ) * model_specific_costs["output_tokens"]
                current_turn_hypothetical_cost += (
                    getattr(usage_info, "cache_creation_input_tokens", 0)
                    / 1_000_000
                ) * model_specific_costs["input_tokens"]
                current_turn_hypothetical_cost += (
                    getattr(usage_info, "cache_read_input_tokens", 0)
                    / 1_000_000
                ) * model_specific_costs["output_tokens"]
                ...
                for (
                    ...
                    model_specific_costs.keys()
                ):  # Iterate through defined cost keys to update usage
                    tokens_in_turn = getattr(usage_info, token_type_key, 0)
                    if (
                        tokens_in_turn > 0
                    ):  # Only update usage if tokens were actually used for this type
